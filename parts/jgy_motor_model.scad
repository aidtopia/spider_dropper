module jgy_motor(angle=0) {
    ...
    jgy_gbox_r = 2;
    jgy_gbox_offset = jgy_motor_offset - jgy_gbox_l;

    translate([0, 0, -jgy_mount_h]) {
        translate([jgy_gbox_offset, -jgy_gbox_w/2, -jgy_gbox_h]) {
            translate([jgy_gbox_l, jgy_gbox_w - jgy_motor_d/2 - 2, jgy_gbox_h - jgy_motor_d/2 - 1]) rotate([0, 90, 0])
        ...
        linear_extrude(jgy_base_h) {
            ...
            hull() {
                translate([12, -14.8/2]) circle(d=6, $fn=12);
                translate([12,  14.8/2]) circle(d=6, $fn=12);
            }
        }
        ...
        linear_extrude(jgy_mount_h, convexity=8) {
            difference() {
                union() {
                    translate([jgy_mount_dy1, -jgy_mount_dx1/2]) circle(d=7.5, $fn=16);
                    translate([jgy_mount_dy1,  jgy_mount_dx1/2]) circle(d=7.5, $fn=16);
                    translate([jgy_mount_dy2, -jgy_mount_dx2/2]) circle(d=7.5, $fn=16);
                    translate([jgy_mount_dy2,  jgy_mount_dx2/2]) circle(d=7.5, $fn=16);
                }
                translate([jgy_mount_dy1, -jgy_mount_dx1/2]) circle(d=3, $fn=12);
                translate([jgy_mount_dy1,  jgy_mount_dx1/2]) circle(d=3, $fn=12);
                translate([jgy_mount_dy2, -jgy_mount_dx2/2]) circle(d=3, $fn=12);
                translate([jgy_mount_dy2,  jgy_mount_dx2/2]) circle(d=3, $fn=12);
            }
        }
